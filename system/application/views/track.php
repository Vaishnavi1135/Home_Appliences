

<div class="container-fluid pt-4 pb-4">
                <img class="img-fluid" src="<?php echo base_url();?>\assets\images\consignment_tracking_jpg_0_0_cover_70.jpg" alt=""  style="height:600px; width:1800px">
                <div class="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center" style="background: cover;">
                    <div class="container">
                        <div class="row justify-content-start" style="float:right; ">
                         <form action="<?php echo base_url('admin/bookings/view');?>" class="container" style="pt-5" >
                        <h3 style="color:white;">Track your belongings with us!</h3>
                        <h6 style="color:white;">To get the location updates of your consignment, <br>
                        write the Goods Consignment (GC) number in the box given below:</h6>

                        <label for="email"><b>GC Number</b></label>
                        <input type="number" placeholder="Enter GC Number" name="GC NUmber"  required>
                
                <input href="<?php echo base_url('admin/bookings/view');?>" type="submit" value="track" style="background:red; color:white;">
            </form>           
        </div>
         </div>
        </div>
    </div>
            